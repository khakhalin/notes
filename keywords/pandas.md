...
* **logical indexing** `df.loc[d.x>0]` Can be combined with list comprehensions; can be written to; but slower, and harder to read.
* **queries**: `df.query('x>0')`, where `x` is a name of a column. Easier for a human to read; works slightly faster, but cannot be written to.
...
There's also`df.apply(@fun, axis=1)`, and it appears that with it one can write a function that is applied to every row of a dataframe, but I'm not quite sure.
...
* A simplified wrapper for **adding homogeneous rows**: `f = f.append(f2)`
