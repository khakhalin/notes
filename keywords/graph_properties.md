...
**Graph diameter**: Maximum shortest path distances between any 2 points in the graph. $\max(h_{ij}) = \max(\min(length(path: i → j)))$. The distance between two farthest points in the graph. Upper bound. Not very useful for giant graphs, as it is fragile (susceptible to a tentacle).
