...
## Transformers
An alternative to RNNs (summary: [1](https://towardsdatascience.com/transformers-141e32e69591)). Text is inherently a stream of signals (words), organized in time, so it makes sense that it should be analyzed by **RNNs**. The problem is that text has long-ish connections (say, objects are elliptically passed from one sentence to another), and RNNs have problems with that (forget, or rather, don't even learn). Then **LSTM** was invented, which improves things, but not enough, as the context still tends to decay exponentially. Also, hard to parallelize, as inputs need to be processed word-by-word. Then they came up with **attention**, by assigning every word a certain "hidden state", and passing it to the entire line (_??? I really not sure how it works_), but still it didn't help enough. The lack of parallelization was the main bottleneck.

But convolutional networks that process info in chunks are highly parallelizable! And the information depth is lower (instead of going through all N elements, activation from the furthest element climbs up the hirarchy through just log N layers). Transformers combine CNNs with attention, and consist of several **encoders** and several **decoders**, all with the same architecture. Encoders contain consist of **Self-Attention** block followed by **FF network**. First encoder receives words, and embeds them in a vector (say, length 512). Self-attention transforms them with 3 trainable matrices in to 3 smaller vectors (64) **Query, Key, and Value**. Then, for each pair of words ij, we calculate s_ij = ⟨qi,kj⟩, scale it down roughly (s_ij/8), and softmax (now ∑s_ij = 1). These values now quantify interactions between the words. Then we calculate the weighted sum of values of each word related to the target word: z_i = ∑ s_ij ∙ v_j, and make this vector of {z_i} an output of self-attention layer that is sent to the FF network. In practice of course alll this vector multiplication happens as one matrix.

But on top of that, they also have **Multihead Attention**, which is like doing the same calculation describe above with different sets of QKV matrices in parallel, to answere different types of questions. And also they encode word positions.

> I'm assuming here that multi-head means "1 representation, several outputs", but this particular document actually doesnt make it clear, so clarify.

Decoders have Self-Attention, followed by **Encoder-Decoder Attention**, followed by FF network.
