...
$\displaystyle \frac{∂J}{∂w^{10}_ i} = \frac{∂J}{∂a^0} \frac{∂a^0}{∂z} \frac{∂z}{∂w^{10}_ i} = 2(a-y) h'(z) a^1_i$

So like in Hebbian rule: if (a0-y) is large, and a1_i is large, then w10_i matters, and gets changed.
...
$\displaystyle \frac{∂J}{∂w^{21}_ j} = 2(a-y)h'(z)w^{10}_ i h'(z^1_ i) a^2_ j$
...
And so on; we can now do it for layers. What's important is that we essentially take the end error derivative 2(a-y), scale it by h'(z); then multiply by the same matrix of w10 (in this example - a vector m×1) that was used on the forward pass, to get "error-effects" at the previous layer 1. Then same way we took the vector of "error-derivatives" (gradient) from layer 1, scale each element by its h'(z1), multiply it by a transposed matrix w21, and get "error-effects" at layer 2. Backprop!
...
* https://cs.nyu.edu/~fergus/papers/zeilerECCV2014.pdf - Visualizing and Understanding
Convolutional Networks. 2014. Zeiler
