...
For many classes, each of log-odds for class g_i is fit linearly: log P(x∈gi)/P(x∈gk) = xᵀθi. Here gk is some reference class (the last one on the list?); it doesn't matter which one exactly; what matters is that with these formulas we get ∑P(gi)=1.

To find a solution, use max-likelihood (maximize conditional P(g|x) ), which is equivalent to minimizing **logloss**. For two classes, Log-Loss = -∑(y log(P(g|x)) + (1-y)log(1-P(g|x))), where y∈{0,1} are true labels. As p→0, −log(p)→inf, so we have a huge punishment for near-zero p. Because of that, when the data is too clean and some areas only contain points of one type, weights can explode (it's hard to fit infinity!), making  **regularization** extremely important. 
